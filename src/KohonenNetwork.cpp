    ...
    : sizeX_(sizeX), sizeY_(sizeY), sizeZ_(sizeZ), input_dim_(input_dim)
{
  ...
  // Inicialización paralela de pesos
#pragma omp parallel for
  for (int i = 0; i < total_neurons; ++i)
  {
    for (int j = 0; j < input_dim_; ++j)
    {
      weights_[i][j] = static_cast<float>(rand()) / RAND_MAX;
    }
  }
}
...
// Usamos critical por seguridad (alternativamente locks por neurona)
